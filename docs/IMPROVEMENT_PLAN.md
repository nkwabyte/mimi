...
| P0 | `--yes` makes every `confirm` return success, including Trash, Docker volumes, stale virtual devices, and orphan deletion. This conflicts with the documentation's stronger confirmation language. | `confirm`, lines 646-654 | Add confirmation classes. `--yes` may skip safe prompts only; risky/irreversible work requires `--force-risky` plus an exact plan ID or interactive confirmation. |
